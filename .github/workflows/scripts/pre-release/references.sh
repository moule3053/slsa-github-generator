...
results=$(
    find .github/workflows/ -maxdepth 1 -name '*.yaml' -o -name '*.yml' -type f -print0 |
        xargs -0 grep -Pn "moule3053/slsa-framework/slsa-github-generator/.*@(?!$RELEASE_TAG)" |
        sed 's/\(.*:\) *uses:.*\(\/.*\)/\1 [...]\2/' ||
        true
)
if [[ "$results" != "" ]]; then
...
results=$(
    find . -name "*.md" -print0 |
        xargs -0 grep -Pn "uses: moule3053/slsa-framework/slsa-github-generator/.*@(?!<|$RELEASE_TAG)" |
        sed "s/\(.*:\) *uses:.*\(\/.*\)/\1 [...]\2/" ||
        true
)
